...
function WidgetkitMapsObj(id, map) {
	...
	this.scrollwheel = map.scrollwheel;
...
WidgetkitMapsExScrollTimer = null;
WidgetkitMapsExscrollDelta = 0;
WidgetkitMapsExscrolling = false;

window.addEventListener(
	'scroll',
	function() {
		if(WidgetkitMapsExScrollTimer != null) {
			clearTimeout(WidgetkitMapsExScrollTimer);
			WidgetkitMapsExScrollTimer = null;
		}

		if(!WidgetkitMapsExscrolling) {
			for (var i=0; i<WidgetkitMaps.length; i++) {
				WidgetkitMaps[i].map.setOptions(
					{
						scrollwheel: false,
						draggable:   false
					}
			...
			WidgetkitMapsExscrolling = true;
		}

		WidgetkitMapsExscrollDelta = window.pageYOffset || document.documentElement.scrollTop;

		WidgetkitMapsExScrollTimer = setTimeout(
			function() {
				var cPos = window.pageYOffset || document.documentElement.scrollTop;

				if(WidgetkitMapsExscrollDelta - cPos == 0) {
					for (var i=0; i<WidgetkitMaps.length; i++) {
						WidgetkitMaps[i].map.setOptions(
							{
								scrollwheel: WidgetkitMaps[i].scrollwheel,
								draggable:   WidgetkitMaps[i].draggable
							}
						);
					}
